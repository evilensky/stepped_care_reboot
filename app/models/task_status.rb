...
class TaskStatus < ActiveRecord::Base
  ...
  scope :for_participant, lambda { |p|
    joins(:membership)
    .where(memberships: { participant_id: p.id })
  }
  scope :is_completed_for_participant, lambda { |p| where
    .not(completed_at: nil)
    .for_participant(p)
  }
  scope :to_be_completed_by_participant, lambda { |p|
    where(completed_at: nil)
    .for_participant(p)
  }
  scope :today, lambda { |p|
    joins(:task)
    .where("tasks.release_day = ?", p.memberships.first.day_in_study)
    .to_be_completed_by_participant(p)
  }
