# This view displays emotions related to the mood selected in the previous page
class ContentProviders::EditPastFeelProvider < BitPlayer::ContentProvider
  def render_current(options)
    ...
    if mood.rating < 0
      mood_related_emotions = [
        "Anger", "Exasperation", "Rage",
        "Disgust", "Envy", "Torment", "Sadness",
        "Sadness", "Disappointment", "Shame",
        "Neglect", "Fear", "Nervousness"]
      valence = -1
    elsif mood.rating == 0
      mood_related_emotions = ["Longing", "Surprise", "Sympathy"]
      valence = 0
    else
      mood_related_emotions = [
